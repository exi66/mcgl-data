...
export const usePostsSearchStore = create<SearchState>((set, get) => ({
  query: "",
  inputValue: "",
  items: [],
  page: 0,
  ...
  fetchNextPage: async () => {
    ...
    try {
      ...
      const arrayColumns = ["usersInText", "linksInText"]

      Object.entries(filters).forEach(([key, value]) => {
        if (key === "query" || !value) return
        if (arrayColumns.includes(key)) {
          const arrayValue = Array.isArray(value) ? value : [value]
          supabaseQuery = supabaseQuery.contains(key, arrayValue)
